...
/***************************************************************************
 EcoVisiol8
                                 A QGIS plugin
 Este complemento fue diseñado para para automatizar y optimizar la obtención de índices SAVI, NDVI y SIPI, así como la realización de correcciones atmosféricas en imágenes Landsat 8.
...
from .eco_visiol8_dialog import EcoVisiol8Dialog
import os.path
...
class EcoVisiol8:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&EcoVisioL8')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('EcoVisiol8', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/eco_visiol8/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Calculate EcoVisioL8'),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&EcoVisioL8'),
                action)
            self.iface.removeToolBarIcon(action)

    def inst(self):
        self.dialogo.etiqueta.setText("\tINSTRUCCIONES\n\n1. Asegurarse que las imágenes satelitales que usará sean exclusivamente de Landsat 8.\n2. Determinar si el proceso requiere corrección atmosférica:\n\nEn caso de requerir:\n\t- Dejar desmarcado la opción “Corrección realizada”.\n\t- Pulsar el icono de la opción “Seleccionar imágenes Landsat 8” y seleccionar todas las bandas satelitales de Landsat 8 (debe cerciorarse que estén las bandas 2, 4 y 5).\n\t- Pulsar el icono de la opción “Seleccionar archivo MTL” y escoger el archivo MTL (formato *.txt).\n\nEn caso de no requerir:\n\t- Marcar la opción “Corrección realizada”.\n\t- Pulsar el icono de la opción “Seleccionar imágenes corregidas Landsat 8” y seleccionar todas las bandas satelitales de Landsat 8 (debe cerciorarse que estén las bandas 2, 4 y 5).\n\n3. Pulsar el icono de la opción “Guardar”, escoger el directorio donde desea guardar y escribir el nombre de la carpeta que contendrá los resultados.\n4. Pulsar el botón “Ejecutar” (es normal que el proceso demore un par de segundos).")
        self.dialogo.exec_()
    ...
    def ejecutar(self):
        ...
        if self.opcion_seleccionada[0] == 'TIF':
            ...
            if self.estado_corr == "activo":
                ...
                self.GUARDAR_NDVI_SAVI_SIPI(banda_2,banda_4,banda_5)
                self.message_bar.clearWidgets()
                self.message_bar = self.iface.messageBar()
                self.message_bar.pushMessage("ANUNCIO:", "Se generó exitosamente el cálculo de los índices NDVI, SAVI Y SIPI en formato .TIF", level=Qgis.Success, duration=0)
                self.limpiar()
            ...
            if self.estado_corr == "inactivo":
                ...
                self.GUARDAR_NDVI_SAVI_SIPI(self.ruta_guardar[0] + '/correcciones/correccion_atmosferica_B2.TIF', self.ruta_guardar[0] + '/correcciones/correccion_atmosferica_B4.TIF', self.ruta_guardar[0] + '/correcciones/correccion_atmosferica_B5.TIF')
            ...
            if self.estado_corr == "activo":
                ...
                self.GUARDAR_NDVI_SAVI_SIPI(banda_2,banda_4,banda_5)
                self.guardar_ndvi_ascii('/TIF/NDVI.TIF', '/TIF/SIPI.TIF','/TIF/SAVI.TIF')
                self.message_bar.clearWidgets()
                self.message_bar = self.iface.messageBar()
                self.message_bar.pushMessage("ANUNCIO:", "Se generó exitosamente el cálculo de los índices NDVI, SAVI Y SIPI en formato .TIF y .ASC.", level=Qgis.Success, duration=0)
                self.limpiar()
    ...
    def filtro(self,path,archivo):
        ...
        cont = 2
        pos = array.index('RADIANCE_MULT_BAND_2')
        RADIANCE_MULT_BAND = []
        i = 2
        while(cont < 6):
        ...
        cont = 2
        pos = array.index('RADIANCE_ADD_BAND_2')
        RADIANCE_ADD_BAND = []
        i = 2
        while(cont < 6):
        ...
        cont = 2
        pos = array.index('RADIANCE_MAXIMUM_BAND_2')
        ...
        cont = 2
        pos = array.index('REFLECTANCE_MAXIMUM_BAND_2')
        ...
        self.correccion(2,banda_2,RADIANCE_MULT_BAND[0],RADIANCE_ADD_BAND[0],distancia_tierra_sol,cos_elevacion_solar,ESUN[0])
    ...
    def NDVI(self, path_4, path_5, nombre_salida,banda_corr_4,banda_corr_5):
        dirB4 = path_4 
        dirB5 = path_5
        banda_4 = QgsRasterLayer(dirB4)
        banda_5 = QgsRasterLayer(dirB5)
        salida = self.ruta_guardar[0] + nombre_salida
        entradas = []
        banda_44 = QgsRasterCalculatorEntry()
        banda_44.ref = banda_corr_4
        banda_44.raster = banda_4
        banda_44.bandNumber = 1
        entradas.append(banda_44)
        banda_55 = QgsRasterCalculatorEntry()
        banda_55.ref = banda_corr_5
        banda_55.raster = banda_5
        banda_55.bandNumber = 1
        entradas.append(banda_55)

        calc = QgsRasterCalculator(f"('{banda_corr_5}'-'{banda_corr_4}')/('{banda_corr_5}'+'{banda_corr_4}')", salida, 'GTiff', banda_4.extent(), banda_4.width(), banda_4.height(), entradas)
        calc.processCalculation()


    def SIPI(self, path_2,path_4,path_5,nombre_salida,banda_corr_2,banda_corr_4,banda_corr_5):
        dirB2 = path_2 
        dirB4 = path_4
        dirB5 = path_5
    ...
    def SAVI(self, path_4, path_5, nombre_salida,banda_corr_4,banda_corr_5):
        ...
        calc = QgsRasterCalculator(f"(('{banda_corr_5}'-'{banda_corr_4}')/('{banda_corr_5}'+'{banda_corr_4}' + '{0.5}'))*('{1.5}')", salida, 'GTiff', banda_4.extent(), banda_4.width(), banda_4.height(), entradas)
        calc.processCalculation()
        
        
    def GUARDAR_NDVI_SAVI_SIPI(self,directorio, directorio1, directorio2):
        if self.opcion_seleccionada[0] =='TIF':

            self.NDVI(directorio1, directorio2, "/NDVI.TIF", 'red@1','nir@1')
            self.SIPI(directorio, directorio1, directorio2, "/SIPI.TIF", 'blue@1','red@1','nir@1')
            self.SAVI(directorio1, directorio2, "/SAVI.TIF", 'red@1','nir@1')

            self.iface.addRasterLayer(self.ruta_guardar[0] + '/NDVI.TIF', "NDVI")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/SIPI.TIF', "SIPI")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/SAVI.TIF', "SAVI")
            

        elif self.opcion_seleccionada[0] =='TIF Y ASC':

            self.NDVI(directorio1, directorio2, "/TIF/NDVI.TIF", 'red@1','nir@1')
            self.SIPI(directorio, directorio1, directorio2, "/TIF/SIPI.TIF", 'blue@1','red@1','nir@1')
            self.SAVI(directorio1, directorio2, "/TIF/SAVI.TIF", 'red@1','nir@1')

            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/NDVI.TIF', "NDVI")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/SIPI.TIF', "SIPI")
            self.iface.addRasterLayer(self.ruta_guardar[0] + '/TIF/SAVI.TIF', "SAVI")
    ...
    def guardar_ndvi_ascii(self, ruta1, ruta2, ruta3):
        self.tranformar_ascii(self.ruta_guardar[0] + ruta1 ,'/ASC/NDVI.ASC')
        self.tranformar_ascii(self.ruta_guardar[0] + ruta2 ,'/ASC/SIPI.ASC')
        self.tranformar_ascii(self.ruta_guardar[0] + ruta3 ,'/ASC/SAVI.ASC')
